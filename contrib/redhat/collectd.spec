...
%define with_debug 0%{!?_without_debug:0}
...
%define with_ovs_dropped 0%{!?_without_ovs_dropped:1}
...
%if %{with_ovs_dropped}
%package ovs_dropped
Summary:       Open vSwitch port drop plugin for collectd
Group:         System Environment/Daemons
Requires:      %{name}%{?_isa} = %{version}-%{release}
BuildRequires: yajl-devel
%description ovs_dropped
...
%if %{with_debug}
%define _with_debug --enable-debug
%else
%define _with_debug --disable-debug
%endif
...
%if %{with_ovs_dropped}
%define _with_ovs_dropped --enable-ovs_drop
%else
%define _with_ovs_dropped --disable-ovs_drop
%endif
